...
class AzoleModel:
    ...
    def __init__(self):
        ...
        self.feature_generator = rdFingerprintGenerator.GetAtomPairGenerator(fpSize=2048)
        self.model = None
        self.seed = 42
        ...
        self.stats = {'AUC': 0.9144897808569082,
    ...
    def featurise(self, smiles: list[str], sanitise: bool = False) -> pd.DataFrame:
        return featurise_smiles(smiles=smiles, fp_generator=self.feature_generator, sanitise=sanitise)

    def predict(self, smiles: list[str]):

        if self.model is None:
            raise RuntimeError(f'No model loaded. Please use ".load_model(model_path) before predicting"')

        # Prepare data for prediction
        df = pd.DataFrame(zip(smiles, [sanitise_smiles(smiles=smi, as_mol=False) for smi in smiles]),
        ...
        df_results = pd.DataFrame([(smi, prob[-1]) for smi, prob in zip(features.index, results)],
    ...
    def get_feature_importance(self, smiles: str, img_out: str | None = None):
        ...
        if img_out:
            ...
            weights = {atom: round(weight/max_contrib, 3) for atom, weight in weights.items()}
