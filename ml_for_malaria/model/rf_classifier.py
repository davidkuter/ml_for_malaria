...
class RFFingerprintClassifier:
    ...
    def __init__(
        self, model: RandomForestClassifier, feature_generator, metadata: ModelMeta
    ):
        self.model = model
    ...
    def load(
        ...
        """Load a random-forest run. ``fingerprint`` selects ``models/{fingerprint}/``."""
        ...
        logger.debug(
            f"Loaded random forest model from {model_path} with {metadata.fingerprint}"
        )
