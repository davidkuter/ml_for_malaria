...
| `report` | `compute_test_metrics`, `TrainingReport` markdown, `write_comparison_report`, Tukey HSD comparison plots (`report_plots`) | Training, RDKit |
| `train` | `prepare_training_run`, `train_xgb_classifier`, `train_rf_classifier`, `train_knn_classifier`, `train_logistic_classifier`, `train_chemprop_classifier` (including CheMeleon foundation), `train_smiles_transformer`, `train_monroe_classifier` | Anything needed at **predict** time |
...
Chemprop, CheMeleon (Chemprop + foundation init), and ChemBERTa live behind the `dl` extra. Monroe + TabPFN needs the `monroe` extra (TabPFN) plus an external Monroe checkout (`MONROE_HOME`, Git LFS `checkpoint/weights.pt`). Do **not** import any of them from `ml_for_malaria.train.__init__` — that would pull torch on every `from ml_for_malaria.train import train_xgb_classifier`.

Secrets (`TABPFN_TOKEN`, optional `MONROE_HOME`) belong in gitignored `.local.env` (see `.local.env.example`). `ml_for_malaria.env.load_local_env()` fills unset keys; process env wins.
